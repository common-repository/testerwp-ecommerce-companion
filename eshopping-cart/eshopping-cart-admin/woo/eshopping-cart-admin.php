...
if(!function_exists('eshopping_cart_product_query')){
    function eshopping_cart_product_query($term_id,$prdct_optn){
   $limit_product = get_theme_mod('eshopping_cart_prd_shw_no','20');
    // product filter
...
if(!function_exists('eshopping_cart_product_slide_list_loop')){
function eshopping_cart_product_slide_list_loop($term_id,$prdct_optn){
    $args = eshopping_cart_product_query($term_id,$prdct_optn);
   ...
   <?php };
      ...
      echo __( 'No products found','eshopping-cart' );
    }
    wp_reset_query();
 }
}
 ...
 **********************************************/   
if(!function_exists('eshopping_cart_category_tab_list')){
function eshopping_cart_category_tab_list( $term_id ){
...
/********************************/
if(!function_exists('eshopping_cart_product_cat_filter_default_loop')){
function eshopping_cart_product_cat_filter_default_loop($term_id,$prdct_optn){
    global $product;
    $args = eshopping_cart_product_query($term_id,$prdct_optn);
    $products = wc_get_products( $args );
    if (!empty($products)) {
        ...
        <div <?php post_class('product', $pid); ?>>
          ...
          <div class="texture-product">
               ...
               <div class="texture-product-hover">     
                    <?php 
                      echo eshopping_cart_add_to_cart_url($product);

                      if ( function_exists('eshopping_cart_whish_list_both')){
                      eshopping_cart_whish_list_both($pid);
                    }
                      if(get_theme_mod( 'eshopping_cart_woo_quickview_enable', true )){
                        $eshopping_cart_quickview_tooltip = esc_html(get_theme_mod( 'eshopping_cart_quickview_tooltip_txt' ));
                  ?>
                   <div class="texture-quickview" th-tooltip="<?php echo $eshopping_cart_quickview_tooltip; ?>">
                               <span class="quik-view">
                                   <a href="#" class="opn-quick-view-text" data-product_id="<?php echo esc_attr($pid); ?>">
                                      <span><?php _e('Quick View','eshopping-cart');?></span>
                                   </a>
                                </span>
                    </div>
                  <?php }
                  ...
                  eshopping_cart_add_to_compare_fltr_both($pid);
                }
...
if(!function_exists('eshopping_cart_product_filter_loop')){
function eshopping_cart_product_filter_loop($args){  
  global $product;
   $products = wc_get_products( $args );
    if (!empty($products)) {
    foreach ($products as $product) {
      ...
      if(count($attachment_ids) > '0'){
                $swapclasses='eshopping-cart-swap-item-hover';
        }
       }elseif('slide' === $hover_style){
       
      $attachment_ids = $product->get_gallery_image_ids();
      if(count($attachment_ids) > '0'){
                $swapclasses='eshopping-cart-slide-item-hover';
        }
       } else{
        $swapclasses='';
       }
...
/********************/
if(!function_exists('eshopping_cart_product_list_filter_loop')){
function eshopping_cart_product_list_filter_loop($args){  
    $products = wc_get_products( $args );
    if (!empty($products)) {
    foreach ($products as $product) {
      $pid =  $product->get_id();
...
if(! class_exists('EShopping_Cart_List_Category_Images')){
class EShopping_Cart_List_Category_Images extends Walker_Category {
