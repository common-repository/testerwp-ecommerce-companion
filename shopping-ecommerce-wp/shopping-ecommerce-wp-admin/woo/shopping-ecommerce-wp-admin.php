...
if(!function_exists('shopping_ecommerce_wp_product_query')){
    function shopping_ecommerce_wp_product_query($term_id,$prdct_optn){
   $limit_product = get_theme_mod('shopping_ecommerce_wp_prd_shw_no','20');
    // product filter
...
if(!function_exists('shopping_ecommerce_wp_product_slide_list_loop')){
function shopping_ecommerce_wp_product_slide_list_loop($term_id,$prdct_optn){
    $args = shopping_ecommerce_wp_product_query($term_id,$prdct_optn);
   ...
   <?php };
      ...
      echo __( 'No products found','shopping-ecommerce-wp' );
    }
    wp_reset_query();
 }
}
 ...
 **********************************************/   
if(!function_exists('shopping_ecommerce_wp_category_tab_list')){
function shopping_ecommerce_wp_category_tab_list( $term_id ){
...
/********************************/
if(!function_exists('shopping_ecommerce_wp_product_cat_filter_default_loop')){
function shopping_ecommerce_wp_product_cat_filter_default_loop($term_id,$prdct_optn){
    global $product;
    $args = shopping_ecommerce_wp_product_query($term_id,$prdct_optn);
    $products = wc_get_products( $args );
    if (!empty($products)) {
        ...
        <div <?php post_class('product', $pid); ?>>
          ...
          <div class="texture-product">
               ...
               <div class="texture-product-hover">     
                    <?php 
                      echo shopping_ecommerce_wp_add_to_cart_url($product);

                      if ( function_exists('shopping_ecommerce_wp_whish_list_both')){
                      shopping_ecommerce_wp_whish_list_both($pid);
                    }
                      if(get_theme_mod( 'shopping_ecommerce_wp_woo_quickview_enable', true )){
                        $shopping_ecommerce_wp_quickview_tooltip = esc_html(get_theme_mod( 'shopping_ecommerce_wp_quickview_tooltip_txt' ));
                  ?>
                   <div class="texture-quickview" th-tooltip="<?php echo $shopping_ecommerce_wp_quickview_tooltip; ?>">
                               <span class="quik-view">
                                   <a href="#" class="opn-quick-view-text" data-product_id="<?php echo esc_attr($pid); ?>">
                                      <span><?php _e('Quick View','shopping-ecommerce-wp');?></span>
                                   </a>
                                </span>
                    </div>
                  <?php }
                  ...
                  shopping_ecommerce_wp_add_to_compare_fltr_both($pid);
                }
...
if(!function_exists('shopping_ecommerce_wp_product_filter_loop')){
function shopping_ecommerce_wp_product_filter_loop($args){  
  global $product;
   $products = wc_get_products( $args );
    if (!empty($products)) {
    foreach ($products as $product) {
      ...
      if(count($attachment_ids) > '0'){
                $swapclasses='shopping-ecommerce-wp-swap-item-hover';
        }
       }elseif('slide' === $hover_style){
       
      $attachment_ids = $product->get_gallery_image_ids();
      if(count($attachment_ids) > '0'){
                $swapclasses='shopping-ecommerce-wp-slide-item-hover';
        }
       } else{
        $swapclasses='';
       }
...
/********************/
if(!function_exists('shopping_ecommerce_wp_product_list_filter_loop')){
function shopping_ecommerce_wp_product_list_filter_loop($args){  
    $products = wc_get_products( $args );
    if (!empty($products)) {
    foreach ($products as $product) {
      $pid =  $product->get_id();
...
if(! class_exists('Shopping_Ecommerce_WP_List_Category_Images')){
class Shopping_Ecommerce_WP_List_Category_Images extends Walker_Category {
